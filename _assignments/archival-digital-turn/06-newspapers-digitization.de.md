---
layout: assignment
unit: archival-digital-turn
authors:
  - estelle-bunout
  - marten-during
lang: de
title: "Die Digitalisierung von Zeitungen: Wie ein neues Kapitel aufgeschlagen wird"
cover:
tags:
  - media history
  - web research
  - conversion from analogue to digital
requirements: Fast WIFI connection, scissors and paper
outcomes:
  - "[TO TRANSLATE] Understanding how the process of selection, digitisation and online publishing of newspapers works" 
  - "[TO TRANSLATE] Understanding what changes occur when newspapers are transformed from print to digital files"
  - "[TO TRANSLATE] Understanding the merits of online access to large volumes of digitised newspapers"
documents:
  - tom-tryniski
  - interface-luxembourgensia
order: 6
---

Zeitungen sind seit dem 19. Jahrhundert ein wichtiges Medium. Sie bieten wertvolle Informationen über Ereignisse und Meinungen in vergangenen und gegenwärtigen Gesellschaften. Die groß angelegte Digitalisierung von Zeitungen, die derzeit in zahlreichen (nationalen) Bibliotheken erfolgt, verspricht ein wahres Sammelsurium an Daten für die historische Forschung. Allerdings werden bei der Einrichtung des Online-Zugangs zu digitalisierten Zeitungen bestimmte Entscheidungen getroffen. Außerdem erfolgen Änderungen, die den informativen und artefaktischen Wert einer Quelle beeinflussen. Historiker sollten in der Lage sein, diese Veränderungen zu identifizieren und zu verstehen. Was zu Hause an Ihrem Bildschirm angezeigt wird, unterscheidet sich recht stark von dem, was Sie in einem Archiv in Ihren Händen halten können. Hinzu kommt, dass nationale Bibliotheken den unterschiedlichsten finanziellen, rechtlichen und technischen Sachzwängen ausgesetzt sind, die neben ihrer eigenen institutionellen Geschichte ihre Digitalisierungsstrategie festlegen. Einige Bibliotheken lagern die Verarbeitung und Auswertung von Daten über öffentlich-rechtliche Partnerschaften mit Unternehmen aus. Diese Aufgabe deckt zwei Ansätze ab: 1. Sie sind aufgefordert, die getroffenen Entscheidungen und die für die Digitalisierung und Online-Veröffentlichung verschiedener Zeitungen genutzten Technologien zu bewerten; 2. Sie analysieren die Vielfalt der Berichterstattung und beleuchten die Möglichkeiten des Online-Zugangs zu Zeitungen, indem Sie eine Vergleichsstudie durchführen.

[card](tom-tryniski)

<!-- more -->

<!-- briefing-student -->

### Anweisungen
<!-- section-contents -->

Sehen Sie sich als Einführung in die Zeitungsdigitalisierung diese Clips zu zwei groß angelegten Digitalisierungsprojekten an, d.h. einen Clip zu einem US-Projekt, in dem sämtliche Aspekte des [Chronicling America](https://www.youtube.com/watch?v=LclIm9s7Iho){:target="_blank"}-Projekts und der Digitalisierungsprozess behandelt werden, sowie einen Clip aus Europa, der zur Förderung des [Europeana Newspapers](https://www.youtube.com/watch?v=5G6v00Z8CyQ){:target="_blank"}-Projekts produziert wurde. Sehen Sie sich darüber hinaus den [Clip über den pensionierten Ingenieur Tom Tryniski](https://www.youtube.com/watch?v=KVWDX6oaYCg&feature=youtu.be){:target="_blank"} an, der ein riesiges Zeitungskorpus digitalisiert und dieses kostenlos online bereitstellt.

Stellen Sie sich nun die wesentlichen Fragen, die bei Anwendung der Quellenkritik auf digitalisierte Zeitungen relevant sind:

Auswahl

- Warum wurde diese Sammlung für die Digitalisierung ausgewählt?
- Ist sie vollständig? Welcher Teil der Sammlung ist abgedeckt?
- Ist dieser Teil repräsentativ?

Umwandlung von der analogen in die digitale Quelle

- Wie wurden die Zeitungen verarbeitet? Wurden alle Zeitungstitel auf dieselbe Weise verarbeitet?
- Wie sehen die Originalzeitungen aus? Erscheinen sie in Schwarzweiß oder in Farbe? Welche Farbe hat das Papier? Ist dies auf Ihrem Bildschirm sichtbar?
- Wurde die Digitalisierung auf Basis von Mikrofilmen oder papiergebundenen Exemplaren durchgeführt?
- Welche Farbe erleichtert das Lesen in der digitalen Form?
- Wie berücksichtigt die Digitalisierung Unterschiede zwischen Zeitungen verschiedener Epochen?

Abrufen

- Sind die Zeitungen durchsuchbar? Lassen sich die Inhalte mithilfe einer Volltextsuche durchsuchen?
- Wie ist die Qualität des Prozesses der optischen Zeichenerkennung (OCR)?
- Wurde die Artikelsegmentierung angewandt? Lassen sich auch Werbeanzeigen und Bilder durchsuchen?

**Anweisungen**

Sie werden herausfinden, wie und warum Zeitungen digitalisiert werden und was erforderlich ist, um diese online einzusehen. Bedingt durch die vielfältigen Verfahren für Digitalisierungsprojekte und Webdesigns und den potenziellen Mangel an Erfahrung mit Online-Archiven ist nicht auszuschließen, dass bei Durchführung dieser Aufgabe der Gesamtüberblick verloren geht. 
Um zu veranschaulichen, welche Antworten auf diese Fragen erwartet werden und was die Variablen in der Tabelle bedeuten, haben wir auf Grundlage des Luxemburger Online-Zeitungsarchivs eluxemburgensia.lu und der Zeitung Tageblatt eine Musterantwort verfasst.
- Öffnen Sie diese PDF-Datei [cit](interface-luxembourgensia), drucken Sie sie aus und verwenden Sie sie als Anhaltspunkt für die Suche nach anderen Zeitungen. 

<!-- section -->

### 6.a Wer digitalisiert zu welchem Zweck, und wie wird der Zugang eingerichtet?
<!-- section-contents -->

- Eine Liste weltweiter Initiativen enthält dieser [Wikipedia-Artikel](https://en.wikipedia.org/wiki/Wikipedia:List_of_online_newspaper_archives){:target="_blank"}. Welche Projekte wurden bzw. werden in Ihrem Land durchgeführt?
- Öffnen Sie die Links zu den folgenden Sammlungen digitalisierter Zeitungen, gehen Sie die Inhalte durch und wählen Sie zwei Zeitungen in einer Sprache, die Sie beherrschen.

|     | 1. Einrichtung | 2. Art des Zugangs | 3. Schnittstelle | 4. Merkmale der Sammlung | 5. Metadaten-Titel
| --- | -------------- | ----------------- | ------------ | --------------------------- | -----------------
| [Europeana newspapers](http://www.europeana-newspapers.eu){:target="_blank"} (mehrsprachig) |   |   |   |  
| [E-Newspapers CH](http://www.onlinenewspapers.com/switzerl.htm){:target="_blank"} (FR+DE)   |   |   |   |  
| [Gallica](https://gallica.bnf.fr/html/und/presse-et-revues/presse-et-revues){:target="_blank"}(FR)  |   |   |   |  
| [Retronews](https://www.retronews.fr){:target="_blank"}(FR) |   |   |   |  
| [ProQuest](https://www.proquest.com/products-services/pq-hist-news.html){:target="_blank"}(EN)|   |   |   |  
| [Chronicling America](https://chroniclingamerica.loc.gov){:target="_blank"}(EN)|   |   |   |  
| [Delpher](https://www.delpher.nl){:target="_blank"} (NL)|   |   |   |  
| [ANNO](http://anno.onb.ac.at/anno-suche#searchMode=simple&from=1){:target="_blank"}(DE)|   |   |   |  
| [Zeitungsportal DDR-Presse](http://zefys.staatsbibliothek-berlin.de/ddr-presse/){:target="_blank"} (DE) |   |   |   |

Tragen Sie die Antworten auf die nachfolgenden Fragen in Ihre Vorlage ein:
- Was für eine Einrichtung hat die Digitalisierung durchgeführt? Öffentlich/privat, für gewerbliche/wissenschaftliche Zwecke oder zur Erhaltung?
- Wie können Sie auf diese Sammlungen zugreifen?
- Ist eine Anmeldung erforderlich? Gibt es einen Fernzugang, oder ist der Zugang nur in den Räumlichkeiten der Einrichtung möglich?
- Gibt es eine spezielle Schnittstelle für die digitalisierten Zeitungen, oder besteht eine gemeinsame Schnittstelle mit anderen Quellen wie Büchern und Bildern?
- Woher stammen die Titel? In welchen Sprachen liegen sie vor? Gibt es spezielle Teilsammlungen?
- Welche Informationen bietet die Einrichtung für jeden einzelnen Titel?

<!-- section -->

### 6.b Die Zeitung als historische Quelle
<!-- section-contents -->

Zeitungen werden regelmäßig veröffentlicht und täglich oder wöchentlich gesammelt und berichten über Ereignisse in der Gesellschaft. Die Sammlungen nationaler Bibliotheken enthalten in der Regel eine große Vielfalt an Zeitungen, darunter bisweilen seltene Titel bzw. Sprachen. So finden sich beispielsweise in der Österreichischen Nationalbibliothek Zeitungen, die bis 1918 in der ehemaligen Österreichisch-Ungarischen Monarchie gedruckt wurden. Die polnische Nationalbibliothek wiederum enthält Zeitungen, die die Existenz besonderer kultureller Gemeinschaften der Vergangenheit widerspiegeln, und in Hebräisch, Jiddisch, Russisch und Deutsch verfasst sind. Dieser Artikel auf der [Nachrichtenwebsite Gizmodo über Zeitungen mit langem Erscheinungszeitraum](https://io9.gizmodo.com/the-worlds-oldest-newspapers-still-being-published-tod-1369904730){:target="_blank"} verdeutlicht die Kontinuität bestimmter Zeitungen. Auffällig ist die unterschiedliche Aufmachung alter und aktueller Zeitungen. Ebenso stellt das Lesen einer Zeitung eine Praxis dar, die zu einem Gefühl nationaler Identität beiträgt. Sie vergleichen gedruckte Zeitungen mit digitalen Zeitungen und der Darstellung von Ereignissen im Laufe der Zeit.

- Sammeln Sie Artikel über zwei Ereignisse in gedruckter oder digitaler Form und stellen Sie die Chronologie dieser Ereignisse wieder her.

Begeben Sie sich für das Durchsuchen gedruckter Exemplare in die Bibliothek und sammeln Sie Artikel zu dem Ereignis aus verfügbaren Zeitungen (Beispiel: Wahlen, Sportereignisse, künstlerische Darbietungen wie Konzerte usw.).

Wählen Sie für die digitalisierte Fassung ein historisches Ereignis wie den Untergang der Titanic, die Ermordung des Erzherzogs Franz-Ferdinand von Österreich 1914, die Veröffentlichung der Tagebücher Anne Franks 1952, das Woodstock-Festival 1969, den Watergate-Skandal 1974 oder den Fall der Berliner Mauer 1989.

- Sammeln Sie jeweils Folgendes:
  - drei Artikel, die vor (Antizipation), beim (Beschreibung) und nach (Kommentare, Folgen) dem Ereignis veröffentlicht wurden.
  - Für digitale Quellen: Suchen Sie nach drei Artikeln, in denen dieses Ereignis 10, 20 und 100 Jahre später behandelt wird.
&nbsp;
- Vervollständigen Sie die Tabellen in Ihrer Vorlage.
- Fassen Sie Ihre Ergebnisse in einem kurzen Aufsatz mit rund 500 Wörtern zusammen und orientieren Sie sich dabei an Ihren Antworten auf diese Fragen:
  - Was hat sich in Bezug auf die Wahrnehmung des Ereignisses/der beteiligten Personen/der Situation geändert? (kurzfristig, langfristig)
  - Worin unterscheidet sich das Sammeln von Artikeln aus gedruckten und digitalisierten Zeitungen insbesondere? (vor Ihrem Bildschirm sitzen, sich ins Archiv/Bibliothek begeben)

<!-- section -->

### 6.c Zeitungen und Websites als Wissensquellen
<!-- section-contents -->

Je nach Ihrem Alter und Bildungshintergrund sind Sie an bestimmte Nachrichtenformen gewöhnt, wie Fernsehen, Radio, Zeitungen, Newsfeeds über Ihr Smartphone oder Nachrichtenwebsites. In dieser Aufgabe sollen Sie über die Quellen Ihres Wissens zu aktuellen kulturellen, wirtschaftlichen und politischen Entwicklungen reflektieren.
- Wählen Sie ein aktuelles Thema oder Ereignis und vergleichen Sie, wie dies in einer gedruckten Zeitung, einer Nachrichtenwebsite und einer Radio- und Fernsehreportage dargestellt wird.
- Füllen Sie die Tabelle in Ihrer Vorlage auf Grundlage dieser Schlüsselfragen aus:
  - Warum haben Sie diese spezifischen Nachrichtenquellen ausgewählt?
  - Vertrauen Sie diesen Quellen?
  - Worauf gründet Ihr Vertrauen?

<!-- section -->

### Empfehlungen Lektüre/Video
<!-- section-contents -->

<!-- briefing-teacher -->
