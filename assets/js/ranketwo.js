;(function () {
  ...
  function Ranketwo() {
    ...
    this.transformLinks = function (language) {
      var aTags = d3.selectAll('a')
      this.log(
        'transformLinks(). Checking: ',
        aTags.size(),
        ' links, language:',
        language
      )
          ...
          if (_self.docs[href]) {
            ...
            switch (this.innerHTML) {
              ...
              case 'block':
                ...
                break
              case 'card':
                var card = _self.createCard(doc, language)
            ...
            console.error(
              'document',
              href,
              'has not been found. Please make sure you correctly added it...'
            )
          }
        }
      })
    ...
    this.refAuthorTitleYear = function (doc) {
      return [
        (doc.data.author || '').replace(/[\s,]$/, ''),
        '<em>' + doc.title.replace(/[\s,]$/, '') + '</em>',
        doc.data.year,
      ]
        .filter(function (d) {
    ...
    this.createCard = function (doc, language) {
      ...
      // create body
      var cardBody = card.append('div').classed('media-body', true)
      var title =
        typeof doc.title === 'object' && typeof doc.title[language] === 'string'
          ? doc.title[language]
          : doc.title
      // title
      cardBody
        .append('h5')
        .html('<a href="' + doc.url + '" target="_blank">' + title + '</a>')
      if (doc.data && doc.data.author && doc.data.author.length) {
        cardBody.append('p').text(_self.refAuthorYear(doc))
        ...
        cardBody
          ...
          .text(
            doc.data.embed.author_name +
              ...
              (doc.data.embed.duration
                ? ' (' +
                  Math.floor(doc.data.embed.duration / 60) +
                  ':' +
                  ('0' + (doc.data.embed.duration % 60)).slice(-2) +
                  ')'
                : '')
          )
        if (doc.data.embed.description)
          cardBody.append('p').text(doc.data.embed.description)
      }
      return card
    }
    ...
    this.createBlock = function (doc) {
      ...
      if (doc.data) {
        ...
        if (doc.attachment) {
          blockImage.style(
            'background-image',
            'url(' + _self.relativePath + doc.attachment + ')'
          )
        }
      }
      // create body
